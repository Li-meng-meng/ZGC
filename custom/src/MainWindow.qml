/****************************************************************************
 * ZGC 覆盖层：指示器抽屉毛玻璃化（方案 H）
 * 同路径覆盖上游 qml/QGroundControl/MainWindow.qml（主窗口根文件，入口加载点
 * QGCCorePlugin.cc createRootWindow，经 CustomOverrideInterceptor 改写命中本覆盖，
 * 注册见 custom/CMakeLists.txt custom_qml_overrides）。
 * 任务：A3-20260909-roundH-glass（ZFYZ-41）。仅指示器抽屉（indicatorDrawer Popup）视觉层
 * 变更，窗口结构与交互契约未动：
 *   - 抽屉背景：上游「globalPalette.window 纯色 × 0.85」矩形改为 MultiEffect 背景模糊
 *     （毛玻璃）：ShaderEffectSource 采样 mainWindow.contentItem 中抽屉正后方区域
 *     （visible:false 仅出纹理不显示原图；弹窗挂 overlay、不属 contentItem 子树，采样无
 *     自捕；_backdropRect 于弹窗定位/显隐/尺寸变化时经 mapToItem 刷新），模糊后叠加
 *     globalPalette.window 半透明（0.65）窗底色调和层保证文字对比度，零新增色值
 *   - backgroundRect 仅 opacity 0.85 → 0.65（id/取色/圆角保持上游，展开按钮锚定契约不变）；
...
ApplicationWindow {
    ...
    Popup {
        ...
        // ZGC: 毛玻璃背景——新增 id 与背后采样/刷新逻辑（结构：模糊层＋色调层在前，展开按钮在后不变）— ZFYZ-41
        background: Item {
            id:             indicatorDrawerBackground

            // ZGC: 抽屉正后方区域（mainWindow.contentItem 坐标系）——弹窗定位 x、显隐、尺寸
            // 变化时经 mapToItem 刷新 — ZFYZ-41
            property rect _backdropRect: Qt.rect(0, 0, width, height)

            function _refreshBackdropRect() {
                var pos = indicatorDrawerBackground.mapToItem(mainWindow.contentItem, 0, 0)
                _backdropRect = Qt.rect(pos.x, pos.y, width, height)
            }

            Component.onCompleted: _refreshBackdropRect()
            onVisibleChanged:      _refreshBackdropRect()
            onWidthChanged:        _refreshBackdropRect()
            onHeightChanged:       _refreshBackdropRect()

            // ZGC: 弹窗 x 绑定重算（挂载后定位/窗口尺寸变化）时同步刷新采样区域 — ZFYZ-41
            Connections {
                target: indicatorDrawer
                function onXChanged() { indicatorDrawerBackground._refreshBackdropRect() }
            }

            // ZGC: 背后采样——visible:false 仅出纹理不显示原图；live:true 随底层内容实时刷新 — ZFYZ-41
            ShaderEffectSource {
                id:           drawerBackdropSource
                anchors.fill: parent
                sourceItem:   mainWindow.contentItem
                sourceRect:   indicatorDrawerBackground._backdropRect
                visible:      false
                live:         true
                smooth:       true
            }
            ...
            // 令效果矩形＝抽屉矩形，模糊边缘不外溢 — ZFYZ-41
            MultiEffect {
                anchors.fill:       parent
                source:             drawerBackdropSource
            ...
            Rectangle {
                ...
                opacity:        0.65  // ZGC: 上游 0.85 → 毛玻璃化色调层，两主题文字对比度达标 — ZFYZ-41
            }
